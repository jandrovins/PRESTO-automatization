...
try:
    ...
    for line in ddplan:
        ...
        for i, dml in enumerate(dmlist):
            ...
            prepsubcmd = "prepsubband -ncpus 16 -nsub %(Nsub)d -lodm %(lowdm)f -dmstep %(dDM)f -numdms %(NDMs)d -numout %(Nout)d -downsamp %(DownSamp)d -o %(root)s %(subfile)s" % {
...
try:
    ...
    for fftf in fftfiles:
        searchcmd = "accelsearch -ncpus 16 -zmax %d %s"  % (zmax, fftf)
        print(searchcmd)
...
try:
    ...
    for cand in cands:
        ...
        foldcmd = "prepfold -ncpus 16 -n %(Nint)d -nsub %(Nsub)d -dm %(dm)f -p %(period)f %(filfile)s -o %(outfile)s -noxwin -nodmsearch" % {
